    ...
    if elements_available():
        ...
        try:
            ...
            with elements(f"pw_row_{key}"):
                ...
                mui.Button("Copy", variant="contained")
        except Exception:
            # If anything goes wrong with streamlit-elements, fall back to plain HTML snippet
            st.write(f"{safe_pw}")
            st.components.v1.html(
                f"<div><input id='pw_{key}' value='{safe_pw}' readonly style='width:200px;padding:6px' /> <button onclick=\"navigator.clipboard.writeText(document.getElementById('pw_{key}').value)\">Copy</button></div>",
                height=40,
            )
            return
        ...
        copy_html = f"""
<div>
  <button id='copy_btn_{key}' style='display:none' onclick="navigator.clipboard.writeText('{safe_pw}')">Copy</button>
  <script>
    ...
    (function() {{
      setTimeout(function() {{
        try {{
          ...
          for (let b of btns) {{
            if (b.textContent && b.textContent.trim() === 'Copy') {{
...
<div>
  ...
  <button onclick="navigator.clipboard.writeText(document.getElementById('pw_{key}').value)">Copy</button>
</div>
"""
...
def render() -> None:
        ...
        if st.button("Create user"):
            if not email or not full_name:
                ...
                try:
                    ...
                    st.success(f"User created: {u['email']}")
                    st.info("Temporary password (copy and give to user):")
    ...
    with eng.connect() as conn:
        ...
        for r in rows:
            ...
            if cols[1].button("Reset pw", key=f"reset_{r['id']}"):
                temp = reset_password(r['id'])
                st.info("New temporary password (copy now):")
                _render_copyable_password(temp, key=f"reset_{r['id']}")
            if cols[2].button("Toggle admin", key=f"toggle_{r['id']}"):
